...
This package is used by other higher-level packages. If you just want to load an image and draw it, check out the "sprite.lua package":doc/sprite.textile.
...
Check for any pending OpenGL errors. Raise a Lua error if an OpenGL error was detected. It's a good idea to call this every so often to see if OpenGL calls fail, e.g. "game.lua":doc/game.textile calls it regularly in its game loop.
